...
export default class Scanner {
  ...
  static scan(
    ...
    scanSettings: ScanSettings,
    courseInfo: CourseInfo,
  ): CourseItemScanResult | null {
    // Only scan module links
    if (
      type === ScannableTypes.MODULE_ITEM &&
      item["type"] !== "File" &&
  ...
  private static getScanProperties(
    item: any,
    type: ScannableType,
    baseUrl: string,
    courseInfo: CourseInfo,
    ...
    switch (type) {
      ...
      case ScannableTypes.MODULE_ITEM:
        ...
        properties.text = [item["title"]];
        properties.html = [];
        if (item["type"] === "ExternalTool" || item["type"] === "ExternalUrl")
          properties.text.push(item["external_url"]);
        break;
    ...
    if (
      ...
      console.warn("Null property in scan properties");
  ...
  private static scanText(text: string, scanSettings: ScanSettings): { matches: string[]; previews: PreviewTuple[] } {
    ...
    for (let i = 0; i < searchTerms.length; i++) {
      ...
      const previewRightEnd =
        previewRightStart + maxPreviewLength > text.length ? text.length : previewRightStart + maxPreviewLength;
