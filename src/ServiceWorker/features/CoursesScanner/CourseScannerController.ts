...
export default class CourseScannerController {
  ...
  constructor(
    ...
    scanSettings: CoursesScanSettings,
    coursesScanController: CoursesScanControllerLike,
  ) {
  ...
  async scanCourseItems(): Promise<void> {
    ...
    while (scannables.length > 0 && this.coursesScanController.running) {
      ...
      for (let i = scannables.length - 1; i >= 0; i--) {
        ...
        const scanResults = Scanner.scanItems(
          scannable.items as any[],
          scannable.type,
          scanSettingsForScanner,
          this.courseInfo as CourseInfo,
        );
