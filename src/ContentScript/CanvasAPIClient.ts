...
class GetRequests {
  static async Announcements(courseId: number | string, page: number = 1, perPage: number = 10): Promise<Response> {
    return await HTTPClient.get(
      CanvasAPIClient.formatURL(
        `/courses/${courseId}/discussion_topics?only_announcements=true&page=${page}&per_page=${perPage}`,
      ),
    );
  }
  ...
  static async CourseFiles(
    courseId: number | string,
    onlyNames: boolean = true,
    page: number = 1,
    perPage: number = 1,
  ): Promise<Response> {
    return await HTTPClient.get(
      CanvasAPIClient.formatURL(
        `/courses/${courseId}/files?${onlyNames ? "only[]=names&" : ""}page=${page}&per_page=${perPage}`,
      ),
    );
  }

  static async CoursesUser(): Promise<Response> {
    return await HTTPClient.get(CanvasAPIClient.formatURL("/courses?per_page=100"));
  }
  ...
  static async CoursesAccount(page: number = 1, perPage: number = 10): Promise<Response> {
    return await HTTPClient.get(CanvasAPIClient.formatURL(`/accounts/1/courses?page=${page}&per_page=${perPage}`));
  }
  ...
  static async Pages(
    ...
    return await HTTPClient.get(
      CanvasAPIClient.formatURL(
        `/courses/${courseId}/pages?${includeBody ? "include[]=body&" : ""}page=${page}&per_page=${perPage}`,
      ),
    );
  }
  ...
  static async UsersSelf(): Promise<Response> {
    return await HTTPClient.get(CanvasAPIClient.formatURL("/users/self"));
  }
}

class PutRequests {
  static async Announcement(
    courseId: number | string,
    announcementId: number | string,
    delayedPostAt: string,
  ): Promise<Response> {
    return await HTTPClient.put(CanvasAPIClient.formatURL(`/courses/${courseId}/discussion_topics/${announcementId}`), {
