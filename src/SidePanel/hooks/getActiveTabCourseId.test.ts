...
describe("getActiveTabCourseId", () => {
  it("extracts the course id from a Canvas course page URL", () => {
    expect(getActiveTabCourseId({ url: "https://school.instructure.com/courses/12345" } as chrome.tabs.Tab)).toBe(
      "12345",
    );
  });
  ...
  it("extracts the course id when the user is on a nested course resource", () => {
    expect(
      getActiveTabCourseId({ url: "https://school.instructure.com/courses/9876/assignments/55" } as chrome.tabs.Tab),
    ).toBe("9876");
  });
