    ...
    <>
      <PrimaryCardLayout className="" fullWidth={true}>
        <PrimaryCard fixedWidth={false} minHeight={false} className="w-full">
          <div className="grid grid-cols-1 grid-flow-row start justify-start content-start gap-2">
            ...
            <p className="text-gray-700 text-base text-left mb-4">{`Items found in ${taskData.resultsData.length} course(s).`}</p>
          </div>
            ...
            <div>
              {taskData && taskData.resultsData.length > 0 && (
                <Tooltip.Root>
                  ...
                  <Tooltip.Content
                    className="select-none p-2 bg-white rounded-sm shadow-xl animate__animated animate__fadeIn"
                    sideOffset={0}
                    side="bottom"
                  >
      ...
      >
        ...
        <Toast.Action className="[grid-area:_action]" asChild altText="Close download notification">
          <IconButton
            ...
            className="text-gray-700 hover:text-white hover:bg-gray-700 appearance-none rounded-full"
          >
      ...
      <Dialog.Root open={curDetails !== null} modal={true}>
        <Dialog.Portal>
          ...
          <Dialog.Content
            onInteractOutside={() => setCurDetails(null)}
          ...
          >
            ...
            <Tabs.Root className="flex flex-col w-full h-80" defaultValue="tab1">
              <Tabs.List className="shrink-0 flex border-b border-gray-200" aria-label="View details for course item">
                <Tabs.Trigger
                  value="tab1"
                  className="h-10 flex-1 flex items-center justify-center text-base select-none text-gray-700 hover:text-blue-500 data-[state=active]:text-blue-600 data-[state=active]:shadow-[inset_0_-2px_0_0] data-[state=active]:shadow-current cursor-default"
                >
                  Previews
                </Tabs.Trigger>
                <Tabs.Trigger
              ...
              <Tabs.Content value="tab2" className="grow">
                {curDetails && (
                  <div className="w-full h-full text-sm p-4 flex flex-col break-words rounded-b shadow-inner">
                    <p className="text-base text-gray-700">
                      <span className="font-bold">Name:</span> {curDetails.name}
                    </p>
                    <p className="text-base text-gray-700">
                      <span className="font-bold">ID:</span> {curDetails.id}
                    </p>
                    <p className="text-base text-gray-700">
                      <span className="font-bold">Published:</span> {curDetails.published ? "TRUE" : "FALSE"}
                    </p>
            ...
            <Dialog.Close asChild>
              <IconButton
                animated={false}
                onClick={() => setCurDetails(null)}
                className="text-gray-700 hover:text-white hover:bg-gray-700 absolute top-3 right-3 appearance-none rounded-full"
              >
