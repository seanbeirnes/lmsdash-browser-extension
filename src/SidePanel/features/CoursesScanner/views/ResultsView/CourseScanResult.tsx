...
import { CaretSortIcon, FileIcon, FileTextIcon } from "@radix-ui/react-icons";
  ...
  return (
    <PrimaryCard fixedWidth={false} minHeight={false} className="w-full">
      <Collapsible.Root open={open} onOpenChange={setOpen}>
        ...
        <Collapsible.Content>
          <div className="w-full sm:w-fit flex flex-col sm:flex-row flex-wrap justify-start items-center content-center gap-2">
            {published ? (
              <p className="w-full sm:w-fit px-4 py-1 text-sm text-green-600 bg-green-200 font-bold text-center rounded-full">
                Published
              </p>
            ) : (
              <p className="w-full sm:w-fit px-4 py-1 text-sm text-red-600 bg-red-200 font-bold text-center rounded-full">
                Unpublished
              </p>
            )}
            {courseCode && (
              <p className="w-full sm:w-fit px-4 py-1 text-sm text-gray-400 bg-gray-100 text-center rounded-full">
                <span className="inline-block font-bold">Course Code:</span> {courseCode}
              </p>
            )}
            {sisCourseId && (
              <p className="w-full sm:w-fit px-4 py-1 text-sm text-gray-400 bg-gray-100 text-center rounded-full">
                <span className="inline-block font-bold">SIS ID:</span> {sisCourseId}
              </p>
            )}
            {id && (
              <p className="w-full sm:w-fit px-4 py-1 text-sm text-gray-400 bg-gray-100 text-center rounded-full">
                <span className="inline-block font-bold">ID:</span> {id}
              </p>
            )}
          </div>
          {(items.announcement?.length ?? 0) > 0 && (
            <CourseItemResultsSection
              icon={<MegaphoneSimpleIcon className="w-6 h-6 text-gray-400" />}
              title="Announcements"
          ...
          {(items.assignment?.length ?? 0) > 0 && (
            <CourseItemResultsSection
              icon={<PencilLineIcon className="w-6 h-6 text-gray-400" />}
              title="Assignments"
          ...
          {(items.courseNavLink?.length ?? 0) > 0 && (
            <CourseItemResultsSection
              icon={<LinkSimpleIcon className="w-6 h-6 text-gray-400" />}
          ...
          {(items.discussion?.length ?? 0) > 0 && (
            <CourseItemResultsSection
              icon={<ChatCircleTextIcon className="w-6 h-6 text-gray-400" />}
              title="Discussions"
          ...
          {(items.file?.length ?? 0) > 0 && (
            <CourseItemResultsSection
              icon={<FileIcon className="w-6 h-6 text-gray-400" />}
              title="File Names"
              items={items.file ?? []}
              infoModalCallback={infoModalCallback}
            />
          ...
          {(items.syllabus?.length ?? 0) > 0 && (
            <CourseItemResultsSection
              icon={<ClipboardTextIcon className="w-6 h-6 text-gray-400" />}
              title="Syllabus"
